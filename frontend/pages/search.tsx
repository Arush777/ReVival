...
export default function SearchPage() {
  ...
  const [slResults, setSlResults] = useState<SearchResult[]>([]);
  ...
  // Client-side catalog search for "All" mode
  const catalogResults = useMemo<CatalogProduct[]>(() => {
    if (!isAllMode || !q.trim()) return [];
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#EAEDED" }}>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "20px 16px" }}>
        ...
        <div style={{ marginBottom: "16px" }}>
          ...
          {isAllMode && (
            <div style={{ fontSize: "13px", color: "#555", marginTop: "4px" }}>
              Showing all Amazon products.{" "}
              <button
                onClick={() => router.push(`/search?q=${encodeURIComponent(q)}&mode=second_life`)}
                style={{
                  background: "none",
                  border: "none",
                  color: "#146EB4",
              ...
              >
                Show only Certified Second Life
              </button>
            </div>
          )}
        </div>

        {/* All-mode: catalog product grid */}
        {isAllMode && searched && (
        ...
        {/* Second-life mode: list results */}
        {!isAllMode && (
          <>
            {!loading && searched && slResults.length === 0 && (
              <div
                style={{
                  backgroundColor: "white",
                  borderRadius: "8px",
